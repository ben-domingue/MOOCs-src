### This is the wget command to get all teh data
# wget --user=researcher --password=myClass -A '*Metadata*''export_summary.txt'','first_attempt.csv','first_grade.csv','first_view.csv','last_attempt.csv','last_grade.csv',last_view.csv','n_attempts.csv','time_spent*','time_to*' -m -p -E -k -K -np https://datastage.stanford.edu/researcher/EDUC_353A/exports_5-12/
...
courses<-list(
              ResGeo="EarthSciences_ResGeo202_Spring2015",
...
set.dir<-"/Users/williamlief/Documents/datahouse/moocs/datastage.stanford.edu/researcher/EDUC_353A/exports_5-12/"

tab<-dat<-list()
for (course in unlist(courses)) {
  # course <- "Medicine_MedStats._Summer2015"
  id.list<-L<-list()
  ...
  for (ii in 1:length(lf)) {
    ...
    strsplit(tmp,"_")->tmp
    sapply(tmp,"[",1)->tmp
#    match(pm[,1],tmp)->index #make sure the items are in the right order, this only returns the first match, need all the matches
...
save(dat,file="/Users/williamlief/Documents/Rstudio/TestTheoryMooc/resp_5-12.Rdata")
